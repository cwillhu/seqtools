...
class NextSeq(IlluminaNextGen):
    ...
    def parseSampleSheet(self):
        with open(self.samplesheetFile,'r') as fh:
            ss = fh.readlines()
        d = dict()
        d['nonIndexRead1_numCycles'] = None
        d['nonIndexRead2_numCycles'] = None
        d['index1Length'] = None
        d['index2Length'] = None
        ...
        for i in range(0,len(ss)):
            ...
            if vals[0] == '[Reads]':
                ...
                if vals[1]:
                    ...
                    self.subID = ''
                    self.notify('Seqprep Warning','Missing subID in samplesheet %s, Line %s: %s' % (self.samplesheetFile, i+1, line))
            elif section == 'Reads':
                if vals[0].isdigit():
                    if d['nonIndexRead1_numCycles'] is None:
                ...
                if vals[0] == 'Adapter' or vals[0] == 'AdapterRead2':
                    if self.suppressAdapterTrimming:
                        ...
                        self.notify('Seqprep Warning','Missing adapter sequence in samplesheet %s, Line %s: %s' % (self.samplesheetFile, i+1, line))

            elif section == 'Data' and vals[0] == 'Sample_ID' and inSampleData == False:
                inSampleData = True
                if 'index2' in vals:
                    hasSecondIndexColumn = True  #Note: this column may be present without there being an index in it.
                uniqueFirstColumnStrings = set() 

            elif inSampleData:
                for j in range(0,len(vals)):
                    vals[j] = re.sub(r'[ .)(@-]','_',vals[j]) #replace any illegal characters with underscores

                if len(vals[0]) == 0 and len(vals[1]) > 0:  #ensure both first and second cols contain values
                    vals[0] == vals[1]
                elif len(vals[1]) == 0 and len(vals[0]) > 0:
                    vals[1] == vals[0]
                elif len(vals[0]) == 0 and len(vals[1]) == 0:
                    self.notify('Seqprep Warning','No values in first or second columns of [Data] section in samplesheet %s, Line %s: %s' % (self.samplesheetFile, i+1, line))

                if vals[0] not in uniqueFirstColumnStrings:  #make the names in first column unique (required by bcl2fastq)
                    uniqueFirstColumnStrings.add(vals[0])
                else:
                    k = 2
                    while ('%s_%s' % (vals[0], k) in uniqueFirstColumnStrings):
                        k += 1
                    newStr = '%s_%s' % (vals[0], k) 
                    uniqueFirstColumnStrings.add(newStr)
                    vals[0] = newStr
                
                firstIndex = vals[5]  #make sure index1 is present, and all index1 lengths are all equal
                if not re.match('[AGCT]+$',firstIndex):  #matches from beg. 
                    self.notify('Seqprep Warning','Missing or invalid index in samplesheet %s, Line %s: %s' % (self.samplesheetFile, i+1, line))
                if d['index1Length'] is None:
                    d['index1Length'] = len(firstIndex)
                elif d['index1Length'] != len(firstIndex):
                    raise Exception('Multiple index1 lengths found in samplesheet %s' % self.samplesheetFile)

                if hasSecondIndexColumn:
                    secondIndex = vals[7]
                    if d['index2Length'] == None:  #make sure index2 lengths are equal (may be all zero)
                        d['index2Length'] = len(secondIndex)
                    elif d['index2Length'] != len(secondIndex):
        ...
        if d['index1Length'] is None:             d['index1Length'] = 0
        if d['index2Length'] is None:             d['index2Length'] = 0
        if d['nonIndexRead1_numCycles'] is None:  d['nonIndexRead1_numCycles'] = 0
        if d['nonIndexRead2_numCycles'] is None:  d['nonIndexRead2_numCycles'] = 0
