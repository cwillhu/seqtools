...
try:
    ...
    # for python 2.6 or earlier, use backport
...
class InteropMetadata(object):
    ...
    def parse_RunParameters(self, filepath):
        ...
        reads_dict = root['Setup']
        
        if 'Reads' in reads_dict.keys(): #hiseq format
            Reads = reads_dict.get('Reads')['Read']
            if type(Reads) is not list: Reads = [Reads]
            for read in Reads:
            ...
            specs_dict = reads_dict
        else: #nexseq format
            for field in ['Read1', 'Read2', 'Index1Read', 'Index2Read']:
                if field in reads_dict.keys():
                    self.read_config.append( 
                        {'read_num': field,
                         'cycles': int(reads_dict[field]),
                         'is_index': True if field.startswith("Index") else False
                         } )                    
            specs_dict = root

        self.rta_version = specs_dict.get('RTAVersion', '')
        self.runID = specs_dict.get('RunID', '')
        self.experiment_name = specs_dict.get('ExperimentName', '')        
        rawdate = specs_dict.get('RunStartDate', '')    # format: 130208 YYMMDD
        if rawdate: 
